...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    @php
        ...
        $ownerWa = $owner->whatsapp_link ?: ($owner->phone ? 'https://wa.me/' . preg_replace('/[^0-9]/', '', $owner->phone) : null);
    @endphp
    @if(true)
        ...
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-x-4 gap-y-1.5 mt-2">
                    ...
                    {{-- Teléfono --}}
                    @if($owner->phone)
                    <a href="tel:{{ $owner->phone }}" class="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-1.5 transition-colors">
                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        {{ $owner->phone }}
                    </a>
                    @endif
                    {{-- WhatsApp (link propio o generado desde teléfono) --}}
                    @if($ownerWa)
                    <a href="{{ $ownerWa }}" target="_blank" class="text-sm text-green-600 hover:text-green-700 font-medium flex items-center gap-1.5 transition-colors">
